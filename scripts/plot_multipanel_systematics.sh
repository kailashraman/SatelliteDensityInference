#!/bin/bash
# Six-panel rho150 systematics comparison (c-M relation, SHMR, dynamical mass
# estimator, host mass, LMC selection, mass floor): single task, no arguments.
#
#   sbatch scripts/plot_multipanel_systematics.sh
#
# Requires scripts/compute_quantiles.sh to have already completed for all
# eight SatGen versions this figure reads (Diemer, Diemer_0p12, Zhao,
# mhalf_scatter, m2e12, lmc, lmc_50, mass_floor_7) -- see
# python/plot_multipanel_systematics.py's VERSIONS and module docstring for
# the panel/version/weight-column mapping.
#
# --output below is resolved by SLURM against the submission directory BEFORE
# this script body runs, so scripts/plot_multipanel_systematics_out/ must
...
#SBATCH --job-name=plot_multipanel_systematics
...
#SBATCH --output=scripts/plot_multipanel_systematics_out/slurm-%j.out
...
mkdir -p "${REPO}/scripts/plot_multipanel_systematics_out"
...
exec python -u "${REPO}/python/plot_multipanel_systematics.py"
