#!/bin/bash
# Regenerate paper figures under plots/ from results/, one producer per
# basename registered in python/figure_registry.py.
#
#   sbatch scripts/make_figures.sh
#
# No plot scripts exist yet (that lands over steps 10-22, each registering
# its basename(s) in figure_registry.REGISTRY / TABLE_REGISTRY). Until then
# python/make_figures.py has nothing to produce and only reports which
# basenames are still pending -- this script already runs cleanly with
# nothing to do, rather than being a broken stub the first migrated figure
# also has to fix.
#
# --output below is resolved by SLURM against the submission directory BEFORE
# this script body runs, so scripts/make_figures_out/ must already exist. It
...
#SBATCH --output=scripts/make_figures_out/slurm-%j.out
...
mkdir -p "${REPO}/scripts/make_figures_out"
...
exec python -u "${REPO}/python/make_figures.py" "$@"
