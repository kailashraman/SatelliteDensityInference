...
Usage:
    python concat_Js.py <version> [dwarf]
...
# Expected halo count for this version, so coverage can be checked rather than
# just contiguity. None if the catalog is not migrated (e.g. a legacy version).
n_halos = None
try:
    import h5py
    with h5py.File(config.h5_path(version), 'r') as _f:
        n_halos = _f['virial_mass'].shape[0]
except Exception as exc:                       # noqa: BLE001 - advisory only
    print(f'catalog for {version!r} unavailable ({exc}); '
...
for dwarf in dwarfs:
    ...
    if n_halos is not None and n != n_halos:
        print(f'{dwarf}: concatenation has {n} halos but the {version} catalog '
              f'has {n_halos}; refusing to write')
        had_error = True
        continue

    record = provenance.stamp('python/concat_Js.py', version=version,
                              argv=sys.argv[1:],
